...
def getCommandActions():
    ...
    f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/commands.json','rb')
    j = f.read()
    f.close()
    if j:
...
if __name__ == '__main__':
    ...
    if len(v) != len(set(v)):
        ...
        for x in ACTIONS.values():
            if x in count:
                ...
                count[x] = 1
        xbmcgui.Dialog().notification("Mania TV Guide", "ACTIONS not unique")
    ...
    while True:
        ...
        if main_action == 0:
            ...
            f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/commands.json','wb')
