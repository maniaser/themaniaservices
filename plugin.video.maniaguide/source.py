...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[plugin.video.maniaguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[plugin.video.maniaguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[plugin.video.maniaguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        lock = 'special://profile/addon_data/plugin.video.maniaguide/db.lock'
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[plugin.video.maniaguide] Updating caches...', xbmc.LOGDEBUG)
            ...
            if getData == True:
                xbmcvfs.delete('special://profile/addon_data/plugin.video.maniaguide/category_count.ini')
    ...
    def exportChannelList(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/channels.ini','wb')
    ...
    def exportChannelIdList(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/channel_id_title.ini','wb')
    ...
    def _getChannelList(self, onlyVisible, all=False):
        ...
        if all == False and self.category and self.category != "Any":
            f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/categories.ini','rb')
    ...
    def _setCustomStreamUrl(self, channel, stream_url):
        if stream_url is not None:
            ...
            if ADDON.getSetting("addon.logos") == "true":
                file_name = 'special://profile/addon_data/plugin.video.maniaguide/icons.ini'
    ...
    def _createTables(self):
        ...
        #except sqlite3.OperationalError, ex:
            ...
            xbmc.log("(plugin.video.maniaguide) %s" % detail, xbmc.LOGERROR)
            dialog = xbmcgui.Dialog()
            dialog.notification('plugin.video.maniaguide', 'database exception %s' % detail, xbmcgui.NOTIFICATION_ERROR , 5000)
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.maniaguide'))
    KEY = 'xmltv'
    INI_TYPE_FILE = 0
    INI_TYPE_URL = 1
    INI_FILE = 'addons.ini'
    ...
    def __init__(self, addon, force):
        ...
        if self.xmltvType == XMLTVSource.XMLTV_SOURCE_FILE:
            ...
            if os.path.exists(customFile):
                ...
                xbmc.log('[plugin.video.maniaguide] Use local file: %s' % customFile, xbmc.LOGDEBUG)
                self.xmltvFile = customFile
            else:
                # Probably a remote file
                xbmc.log('[plugin.video.maniaguide] Use remote file: %s' % customFile, xbmc.LOGDEBUG)
        ...
        if (ADDON.getSetting('addons.ini.subscriptions') == "true"):
            file_name = 'special://profile/addon_data/plugin.video.maniaguide/subscriptions.ini'
        ...
        path = "special://profile/addon_data/plugin.video.maniaguide/addons.ini"
        if not xbmcvfs.exists(path):
            f = xbmcvfs.File(path,"w")
            f.close()

        addons_ini = "special://profile/addon_data/plugin.video.maniaguide/addons.ini"
        ...
        if (ADDON.getSetting('addons.ini.subscriptions') == "true") or (ADDON.getSetting('addons.ini.overwrite') == "1"):
            streams = {}
            streams["plugin.video.maniaguide"] = {}
            ...
            if (ADDON.getSetting('addons.ini.subscriptions') == "true"):
                for name in subscription_streams:
                    if name:
                        streams["plugin.video.maniaguide"][name] = subscription_streams[name]
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            try:
                ...
                xbmc.log('[plugin.video.maniaguide] strptime error with this date: %s' % dateString, xbmc.LOGDEBUG)
    ...
    def parseXMLTV(self, context, f, size, logoFolder, progress_callback):
        ...
        data = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/channel_id_shortcut.ini','rb').read()
        id_shortcuts = {}
        if data:
            lines = data.splitlines()
            for line in lines:
        ...
        if ADDON.getSetting('update.progress') == 'true':
            ...
            d.close()
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/category_count.ini',"wb")
