...
def select_countries():
    ...
    if result:
        ...
        countries = ','.join(ids)
        xbmcaddon.Addon(id = 'plugin.video.maniaguide').setSetting('yo.countries',countries)


def select_providers():
    s = xbmcaddon.Addon(id = 'plugin.video.maniaguide').getSetting('yo.countries')
    if not s:
        return
    countries = s.split(',')
    d = xbmcgui.Dialog()
    ...
    xbmcaddon.Addon(id = 'plugin.video.maniaguide').setSetting('yo.%s.headend' % country, headend)
