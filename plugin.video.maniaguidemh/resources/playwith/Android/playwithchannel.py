...
path = xbmc.translatePath('special://profile/addon_data/plugin.video.maniaguidemh/maniahd.db')
try:
    conn = sqlite3.connect(path, detect_types=sqlite3.PARSE_DECLTYPES)
    conn.row_factory = sqlite3.Row
except Exception as detail:
    xbmc.log("EXCEPTION: (plugin.video.maniaguidemh)  %s" % detail, xbmc.LOGERROR)
