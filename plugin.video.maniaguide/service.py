...
class Service(object):
    ...
    def onInit(self, success):
        if success:
            xbmc.log("[plugin.video.maniaguide] Background Update Starting...", xbmc.LOGNOTICE)
    ...
    def onCachesUpdated(self):
        ...
        xbmc.log("[plugin.video.maniaguide] Background Update Finished", xbmc.LOGNOTICE)
        if ADDON.getSetting('background.notify') == 'true':
            d = xbmcgui.Dialog()
            d.notification("Mania TV Guide", "Finished Updating")
if __name__ == '__main__':
    ADDON = xbmcaddon.Addon('plugin.video.maniaguide')

    version = ADDON.getAddonInfo('version')
    if ADDON.getSetting('version') != version:
        #text = xbmcvfs.File('special://home/addons/plugin.video.maniaguide/changelog.txt','rb').read()
    ...
    xbmcvfs.delete('special://profile/addon_data/plugin.video.maniaguide/source.db-journal')
    lock = 'special://profile/addon_data/plugin.video.maniaguide/db.lock'
    xbmcvfs.delete(lock)
    try:
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(plugin.video.maniaguide)")

        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[plugin.video.maniaguide] Background service started...", xbmc.LOGDEBUG)
            if ADDON.getSetting('background.startup') == 'true':
                ...
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/plugin.video.maniaguide/ReloadAddonFolders.py)')
            while not monitor.abortRequested():
                if ADDON.getSetting('service.type') == '0':
                    ...
                    xbmc.log("[plugin.video.maniaguide] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                else:
                ...
                if timeLeft < 0:
                    timeLeft = 0
                xbmc.log("[plugin.video.maniaguide] Service waiting for %d seconds" % timeLeft, xbmc.LOGDEBUG)
                if timeLeft and monitor.waitForAbort(timeLeft):
                    break
                xbmc.log("[plugin.video.maniaguide] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/plugin.video.maniaguide/ReloadAddonFolders.py)')
                now = time.time()
        ...
        xbmc.log('[plugin.video.maniaguide] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
