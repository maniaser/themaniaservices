...
if ADDON.getSetting('skin.source') == "0":
    ...
    SKIN_PATH = xbmc.translatePath("special://profile/addon_data/plugin.video.maniaguide/")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def loadTVDBImages(self):
        file_name = 'special://profile/addon_data/plugin.video.maniaguide/tvdb.pickle'
    ...
    def close(self):
        if not self.isClosing:
            ...
            f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/tvdb.pickle','wb')
            try:
            ...
            file_name = 'special://profile/addon_data/plugin.video.maniaguide/custom_stream_urls_autosave.ini'
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["NEXT_DAY"]:
            ...
            if kodi < 16:
                ...
                f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/categories.ini','wb')
    ...
    def programSearch(self):
        ...
        file_name = "special://profile/addon_data/plugin.video.maniaguide/title_search.list"
    ...
    def descriptionSearch(self):
        d = xbmcgui.Dialog()
        title = ''
        file_name = "special://profile/addon_data/plugin.video.maniaguide/synopsis_search.list"
        f = xbmcvfs.File(file_name,"rb")
        searches = sorted(f.read().splitlines())
        f.close()
        actions = ["New Search", "Remove Search"] + searches
    ...
    def categorySearch(self):
        d = xbmcgui.Dialog()
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/category_count.ini',"rb")
    ...
    def _showContextMenu(self, program):
        ...
        if buttonClicked == PopupMenu.C_POPUP_REMIND:
            ...
            if ADDON.getSetting('search.type') == 'MySearch':
                script = "special://home/addons/plugin.video.maniaguide/search.py"
    ...
    def clear_catchup(self):
        if not self.playing_catchup_channel:
            return
        self.playing_catchup_channel = False
        filename = 'special://profile/addon_data/plugin.video.maniaguide/catchup_channel.list'
    ...
    def catchup(self,channel):
        ...
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguide/catchup_channel.list','wb')
        first_cmd = None
    ...
    def playWithChannel(self, channel, program = None):
        ...
        if url:
            ...
            if ffmpeg:
                ...
                script = "special://home/addons/plugin.video.maniaguide/playwithchannel.py"
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))

            script = "special://profile/addon_data/plugin.video.maniaguide/playwithchannel.py"
            if xbmcvfs.exists(script):
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))
    ...
    def stopWith(self):
        ...
        if ffmpeg:
            folder = ADDON.getSetting('autoplaywiths.folder')
            script = "special://home/addons/plugin.video.maniaguide/stopwithchannel.py"
            xbmc.executebuiltin('RunScript(%s)' % (script))

        script = "special://profile/addon_data/plugin.video.maniaguide/stopwithchannel.py"
        if xbmcvfs.exists(script):
            xbmc.executebuiltin('RunScript(%s)' % (script))
        xbmc.Player().stop()
    ...
    def saveActions(self):
        file_name = 'special://profile/addon_data/plugin.video.maniaguide/actions.json'
        f = xbmcvfs.File(file_name,"wb")
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
        ...
        if controlId == self.C_STREAM_BROWSE_ADDONS:
            ...
            if stream:
                ...
                if len(lines) > 1:
                    ...
                    file_name = 'special://profile/addon_data/plugin.video.maniaguide/addons.ini'
                    ...
                    addonId = "plugin.video.maniaguide"
    ...
    def updateDirsInfo(self):
        file_name = 'special://profile/addon_data/plugin.video.maniaguide/folders.list'
        f = xbmcvfs.File(file_name)
        lines = f.read().splitlines()
        folders = []
        for folder in lines:
