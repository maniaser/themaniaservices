...
def Login():
    ...
    try:
        ...
        dp.create(THESITE.upper(), "CloudFlare.... ", '', 'Please Wait')
        cloudflare.solve(loginurl, cookie_jar, dp)
        net.set_cookies(cookie_jar)
        dp.update(100, "Login into ULTIMATEMANIA.... ", "Please Wait")
        html = net.http_POST(loginurl, data, headers).content
        dp.close()
    ...
    if 'true' in html:
       ...
       if stat:		
            ...
            dialog = xbmcgui.Dialog()
            dialog.ok(THESITE.upper(), '','Your subscription has expired please go to http://ultimatemania.rocks/payments/login to resubscribe to ULTIMATE MANIA','')
            try:os.remove(cookie_jar)
            except:pass
            SYSEXIT()
            return False
...
def parseAPI(url):
       ...
       if len(str(nameSelect))> 7:
           ...
           if '[COLOR yellow]Search[/COLOR]' in str(GENRE):
               keyboard = xbmc.Keyboard('', 'Search On Demand')
               keyboard.doModal()
               if keyboard.isConfirmed():
                   search_entered = keyboard.getText()
...
def CheckUpdate():
    dialog = xbmcgui.Dialog()
    link=OPEN_URL('https://raw.githubusercontent.com/Willowsportsmania/themaniaservices/master/plugin.video.ultimatemania/addon.xml')
