...
def getLogo(title,ask=False,force=True):
    infile = xbmc.translatePath("special://profile/addon_data/plugin.video.maniaguidesm/logos/temp.png")
    outfile = xbmc.translatePath("special://profile/addon_data/plugin.video.maniaguidesm/logos/%s.png" % title)
    if not force and xbmcvfs.exists(outfile):
        return outfile
    xbmcvfs.mkdirs("special://profile/addon_data/plugin.video.maniaguidesm/logos")
    ...
    if json and "channels" in json:
        ...
        if channels:
            ...
            if selected > -1:
                ...
                f = xbmcvfs.File("special://profile/addon_data/plugin.video.maniaguidesm/logos/temp.png","wb")
                f.write(data)
                f.close()
                from PIL import Image, ImageOps
