...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.video.maniaguidesm'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self,force=False):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_DEFAULT:
                xbmc.log('[plugin.video.maniaguidesm] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                ...
                if not xbmcvfs.copy(self.fileUrl, tmpFile):
                    xbmc.log('[plugin.video.maniaguidesm] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                ...
                if self.addon.getSetting('md5') == 'true':
                    ...
                    try:
                        ...
                        xbmc.log('[plugin.video.maniaguidesm] Missing md5: %s.md5 (%s)' % (self.fileUrl,detail), xbmc.LOGERROR)
                    #log((old_md5,new_md5))
                    if old_md5 and (old_md5 == new_md5) and (self.addon.getSetting('xmltv.refresh') == 'false'):
                        return self.FETCH_NOT_NEEDED
                f = open(tmpFile, 'wb')
                xbmc.log('[plugin.video.maniaguidesm] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
                total = 0
                ...
                try:
                    ...
                    if r.status_code != requests.codes.ok:
                        if ADDON.getSetting('gz') == 'true':
                            ...
                            if r.status_code != requests.codes.ok:
                                xbmc.log('[plugin.video.maniaguidesm] no file: %s' % self.fileUrl, xbmc.LOGERROR)
                                xbmcgui.Dialog().notification("Mania TV Guide", "bad status code %s" % self.fileUrl,xbmcgui.NOTIFICATION_ERROR)
                        else:
                            xbmc.log('[plugin.video.maniaguidesm] no file: %s' % fileUrl, xbmc.LOGERROR)
                            xbmcgui.Dialog().notification("Mania TV Guide", "bad status code %s " % fileUrl,xbmcgui.NOTIFICATION_ERROR)
                            return self.FETCH_NOT_NEEDED
                    if "Content-Length" in r.headers:
                        total = int(r.headers['Content-Length'])
                except Exception as detail:
                    xbmc.log('[plugin.video.maniaguidesm] bad request: %s (%s)' % (fileUrl,detail), xbmc.LOGERROR)
                    xbmcgui.Dialog().notification("Mania TV Guide", "failed to download %s " % fileUrl,xbmcgui.NOTIFICATION_ERROR)
                    return self.FETCH_NOT_NEEDED

                title = fileUrl.split('/')[-1]
                d = xbmcgui.DialogProgressBG()
                d.create('Mania TV Guide', 'downloading %s' % title)
                chunk_size = 16 * 1024
                size = 0
                oldtime = time.time()
            ...
            if new_md5 and (self.addon.getSetting('md5') == 'true'):
                ...
                if md5_file != new_md5:
                    xbmc.log('[plugin.video.maniaguidesm] md5 mismatch: %s calculated:%s server:%s' % (self.fileUrl,md5_file,new_md5), xbmc.LOGERROR)
                    xbmcgui.Dialog().notification("Mania TV Guide", "failed md5 check %s",xbmcgui.NOTIFICATION_ERROR)
                else:
                    xbmcvfs.File(self.filePath+".md5","wb").write(new_md5)
            retVal = self.FETCH_OK
            xbmc.log('[plugin.video.maniaguidesm] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
        return retVal