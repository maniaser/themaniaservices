...
if ADDON.getSetting('skin.source') == "0":
    ...
    SKIN_PATH = xbmc.translatePath("special://profile/addon_data/plugin.video.maniaguidemh/")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __init__(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/categories.ini','rb')
    ...
    def loadTVDBImages(self):
        file_name = 'special://profile/addon_data/plugin.video.maniaguidemh/tvdb.pickle'
    ...
    def getControl(self, controlId):
        ...
        try:
            ...
            #xbmc.log("EXCEPTION: (plugin.video.maniaguidemh) TVGuide.getControl %s" % detail, xbmc.LOGERROR)
    ...
    def close(self):
        if not self.isClosing:
            ...
            f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/tvdb.pickle','wb')
            try:
            ...
            file_name = 'special://profile/addon_data/plugin.video.maniaguidemh/custom_stream_urls_autosave.ini'
    ...
    def ChooseStreamAddon(self, result, channel):
        ...
        if url:
            if url.startswith('plugin://'):
                ...
                name = "Playlist"
                icon = xbmcaddon.Addon('plugin.video.maniaguidemh').getAddonInfo('icon')
        stream = ""
        title = ""
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["NEXT_DAY"]:
            ...
            if results:
                if len(results) > 1:
                    ...
                    if what > -1:
                        ...
                        xbmcgui.Dialog().notification("Mania TV Guide", "Couldn't find: %s" % title)
                else:
                    if selection == 0:
            ...
            if kodi < 16:
                dialog.ok('Mania TV Guide', 'Editing categories in Kodi %s is currently not supported.' % kodi)
            else:
                ...
                f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/categories.ini','wb')
            ...
            xbmc.log('[plugin.video.maniaguidemh] Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def onActionQuickEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["LEFT"]:
            ...
            xbmc.log('[plugin.video.maniaguidemh] quick epg Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def programSearch(self):
        ...
        file_name = "special://profile/addon_data/plugin.video.maniaguidemh/title_search.list"
    ...
    def descriptionSearch(self):
        d = xbmcgui.Dialog()
        title = ''
        file_name = "special://profile/addon_data/plugin.video.maniaguidemh/synopsis_search.list"
        f = xbmcvfs.File(file_name,"rb")
        searches = sorted(f.read().splitlines())
        f.close()
        actions = ["New Search", "Remove Search"] + searches
    ...
    def categorySearch(self):
        d = xbmcgui.Dialog()
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/category_count.ini',"rb")
    ...
    def _showContextMenu(self, program):
        ...
        if buttonClicked == PopupMenu.C_POPUP_REMIND:
            ...
            if ADDON.getSetting('search.type') == 'MySearch':
                script = "special://home/addons/plugin.video.maniaguidemh/search.py"
    ...
    def clear_catchup(self):
        if not self.playing_catchup_channel:
            return
        self.playing_catchup_channel = False
        filename = 'special://profile/addon_data/plugin.video.maniaguidemh/catchup_channel.list'
    ...
    def catchup(self,channel):
        ...
        f = xbmcvfs.File('special://profile/addon_data/plugin.video.maniaguidemh/catchup_channel.list','wb')
        first_cmd = None
    ...
    def playWithChannel(self, channel, program = None):
        ...
        if url:
            ...
            if ffmpeg:
                ...
                script = "special://home/addons/plugin.video.maniaguidemh/playwithchannel.py"
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))

            script = "special://profile/addon_data/plugin.video.maniaguidemh/playwithchannel.py"
            if xbmcvfs.exists(script):
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))
    ...
    def stopWith(self):
        ...
        if ffmpeg:
            folder = ADDON.getSetting('autoplaywiths.folder')
            script = "special://home/addons/plugin.video.maniaguidemh/stopwithchannel.py"
            xbmc.executebuiltin('RunScript(%s)' % (script))

        script = "special://profile/addon_data/plugin.video.maniaguidemh/stopwithchannel.py"
        if xbmcvfs.exists(script):
            xbmc.executebuiltin('RunScript(%s)' % (script))
        xbmc.Player().stop()
    ...
    def saveActions(self):
        file_name = 'special://profile/addon_data/plugin.video.maniaguidemh/actions.json'
        f = xbmcvfs.File(file_name,"wb")
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
        ...
        if controlId == self.C_STREAM_BROWSE_ADDONS:
            ...
            if stream:
                ...
                if len(lines) > 1:
                    ...
                    file_name = 'special://profile/addon_data/plugin.video.maniaguidemh/addons.ini'
                    ...
                    addonId = "plugin.video.maniaguidemh"
            ...
            if item:
                ...
                d = xbmcgui.Dialog()
                d.notification("Mania TV Guide", title, sound=False, time=500)
    ...
    def updateDirsInfo(self):
        file_name = 'special://profile/addon_data/plugin.video.maniaguidemh/folders.list'
        f = xbmcvfs.File(file_name)
        lines = f.read().splitlines()
        folders = []
        for folder in lines:
    ...
    def addBrowseFolder(self):
        ...
        file_name = 'special://profile/addon_data/plugin.video.maniaguidemh/icons.ini'
