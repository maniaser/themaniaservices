...
path = xbmc.translatePath('special://profile/addon_data/plugin.video.maniaguidesm/sportsmania.db')
try:
    conn = sqlite3.connect(path, detect_types=sqlite3.PARSE_DECLTYPES)
    conn.row_factory = sqlite3.Row
except Exception as detail:
    xbmc.log("EXCEPTION: (plugin.video.maniaguidesm)  %s" % detail, xbmc.LOGERROR)
...
if url:
    ...
    dialog.notification("Mania TV Guide","Starting Recording",sound=True)
    ...
    dialog.notification("Mania TV Guide","Finished Recording",sound=True)