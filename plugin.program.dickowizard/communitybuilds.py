...
AddonTitle   =  "[COLOR=white]The Community Wizard[/COLOR]"
...
def Archive_Tree(sourcefile, destfile, message_header, message1, message2, message3, exclude_dirs, exclude_files):
    ...
    for base, dirs, files in os.walk(sourcefile):
        ...
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.dickowizard' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dp.create("[COLOR=white]The Community Wizard[/COLOR]","Archiving...",'', 'Please Wait')
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ITEM.append(file)
    N_ITEM =len(ITEM)
...
def Check_Download_Path():
    ...
    if not os.path.exists(zip):
        dialog.ok('[COLOR=white]The Community Wizard[/COLOR]','The download location you have stored does not exist .\nPlease update the addon settings and try again.','','')        
...
def Community_Backup():
    ...
    exclude_dirs =  ['plugin.program.dickowizard', 'plugin.repository.shadowcrew','cache', 'system', 'Thumbnails', "peripheral_data",'library','keymaps']
...
def Fix_Special(url):
    dp.create("[COLOR=white]The Community Wizard[/COLOR]","Renaming paths...",'', 'Please Wait')
...
def Install_Art(path):
    ...
    try:
        ...
        time.sleep(1)
#        Read_Zip(artpack)
        dp.create("[COLOR=white]The Community Wizard[/COLOR]","Checking ",'', 'Please Wait')
...
def Restore_Backup_XML(name,url,description):
    if 'Backup' in name:
        ...
        if 'guisettings.xml' in description:
            ...
            f.write(TO_READ)
            f.close()  
    dialog.ok("[COLOR=white]The Community Wizard[/COLOR]", "", 'All Done !','')
#---------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------
...
def Restore_Local_Community():
    ...
    if exitfunction == 1:
        ...
        dialog.ok('[COLOR=white]The Community Wizard[/COLOR]','Step 1 complete. Now please change the skin to','the one this build was designed for. Once done come back','to this addon and restore the guisettings_fix.zip')        
        xbmc.executebuiltin("ActivateWindow(appearancesettings)")
#---------------------------------------------------------------------------------------------------
...
def Restore_Zip_File(url):
    ...
    if 'Backup' in name:
        Delete_Packages() 
        dp.create("[COLOR=white]The Community Wizard[/COLOR]","Backing Up",'', 'Please Wait')
        ...
        zipobj.close()
        dp.close()
        dialog.ok("[COLOR=white]The DTG Wizard[/COLOR]", "You Are Now Backed Up", '','')   
    else:
        dp.create("[COLOR=white]The DTG Wizard[/COLOR]","Checking ",'', 'Please Wait')
        dp.update(0,"", "Extracting Zip Please Wait")
        ...
        if 'Backup' in name:
            extras.Kill_XBMC()
            dialog.ok("DTG Builds - Install Complete", 'To ensure the skin settings are set correctly XBMC will now', 'close. If XBMC doesn\'t close please force close (pull power', 'or force close in your OS - [COLOR=lime]DO NOT exit via Kodi menu[/COLOR])')
        else:
            dialog.ok("[COLOR=white]The DTG Wizard[/COLOR]", "You Are Now Restored", '','')        
#---------------------------------------------------------------------------------------------------
